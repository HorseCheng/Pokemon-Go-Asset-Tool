...
shadow = [1,2,3,4,5,6,7,8,9,13,14,15,19,20,23,24,27,28,41,42,43,44,45,48,49,50,51,52,53,54,55,58,59,60,61,62,63,64,65,69,70,71,74,75,76,81,82,88,89,90,91,95,96,97,98,99,104,105,106,107,109,110,111,112,115,123,
          125,126,129,130,131,137,143,144,145,146,147,148,149,169,179,180,181,182,186,198,202,208,212,215,225,228,229,233,234,258,246,247,248,258,259,260,273,274,275,
          280,281,282,302,328,329,330,331,332,353,354,355,356,359,371,372,373,387,388,389,390,391,392,430,443,444,445,449,450,459,460,461,462,464,466,467,474,475,477]
indexlist = []
pokedata = []
...
for i in range(0, len(master)):
    ...
    if x and "SHADOW" not in master[i] and "PURIFIED" not in master[i]:
        ...
        if (
            (num in alola)
            or (num in [25, 150, 386, 479, 493, 646])
            or (num in shadow) or (num in [110,262,263,264])
        ):
        ...
        if (
            ...
            i += 40
            continue
        if("SHADOW" not in master[i] and "PURIFIED" not in master[i]):
...
for e in range(0, len(chi)):  # pokemon introducing type
    ...
    if "pokemon_category" in chi[e] and "0000" not in chi[e]:
        ...
        for tt in range(
            0, pokecheck[num]
        ):  # only print once, special type don't need to print
            if tt == 0:
                intro[2].append(temp)
            else:
                intro[2].append("")
...
for e in range(startt, len(chi)):  # pokemon introducing description
    if "pokemon_evolution_tap_to_evolve" in chi[e]:
        startt = e
        break
        ...
        if duplicated != 1:
            for tt in range(
                ...
                if tt == 0:
                    ...
                    duplicated = 1
                    intro[3].append(tempAlola)  # second line for Alola
                else:
                    intro[3].append("")
        else:
            duplicated = 0
