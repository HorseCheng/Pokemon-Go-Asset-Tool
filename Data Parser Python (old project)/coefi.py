...
for i in range(0, len(master)):
    if 'template_id: "COMBAT_SETTINGS' in master[i]:
        ...
        while "}" not in master[i]:
            if "{" not in master[i]:
                ...
                position = temp.find(":")
                coefi[6] += (
                    temp[0 : position + 1] + " " + temp[position + 1 : len(temp) + 1]
                )
            i += 1
        continue
    ...
    if 'template_id: "PLAYER_LEVEL_SETTINGS' in master[i]:
        while "}" not in master[i]:
            ...
            if (
                "max_egg_player_level" in master[i]
                or "max_encounter_player_level" in master[i]
                or "max_quest_encounter_player_level" in master[i]
            ):
                temp = ""
                temp += master[i]
                temp = temp.replace(" ", "")
...
dd = "Coefficient/"
output = [
    dd + "1_cp_multiplier.txt",
    dd + "2_required_expeirence.txt",
    dd + "3_candy_cost.txt",
    dd + "4_stardust_cost.txt",
    dd + "5_allinone.txt",
    dd + "6_weather.txt",
]
