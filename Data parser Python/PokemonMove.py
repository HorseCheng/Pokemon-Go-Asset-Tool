...
for i in range(0, len(master)):
    ...
    if x:
        if 'SHADOW"' in master [i] or 'PURIFIED"' in master[i]: continue
        ...
        if (
            ...
            or (  num in shadow
            )or (num in [110,262,263,264])
        ):
            if "NORMAL" in master[i] or "ORIGIN" in master[i]: continue
        if num != checknum:
            check = 1
        if (
            num
...
pokechargemove = pokechargemove[:-1]
mewc = mewc[:-1]
pokequickmove = pokequickmove[:-1]
mewq = mewq[:-1]
...
aa.write(pokequickmove)
aa.close()
bb.write(pokechargemove)
bb.close()
cc.write(mewc)
cc.close()
dd.write(mewq)
