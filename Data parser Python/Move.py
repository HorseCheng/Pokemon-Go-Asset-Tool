...
for x in range(len(chi)):
    ...
    if "pokemon_unfavorite_error" in chi[x]:
        break
...
def move_info(index, qorc, position, battletype):
    ...
    if battletype == "gym":
        if qorc == 1:
            stringquick[index] += temp + "\n"
        else:
            stringcharge[index] += temp + "\n"
    else:
        if qorc == 1:
            stringquickpvp[index] += temp + "\n"
        else:
            stringchargepvp[index] += temp + "\n"
...
for i in range(0, len(master)):  # Find the Move info for Gym, Raid
    ...
    if temp:
        if "_FAST" in master[i]:
            qorc = 1
            # qorc: quick attack or charge attack
        else:
            qorc = 0
        ...
        if qorc == 1:  # quick attack
            ...
            for e in range(noww, len(eng)):
                if "move_reroll_confirm_desc" in eng[e]:
                    break
        ...
        checkmatrix = [
            0 for c in range(0, 6)
        ]  # check whether the move information is not showed in Game Master
            ...
            if "pokemon_type:" in master[y]:
                ...
                for z in range(0, len(Key[0])):
                    if temp in Key[0][z]:  # find chinese word
                        if qorc == 1:
                            stringquick[0] += Key[1][z] + "\n"
                        else:
                            stringcharge[0] += Key[1][z] + "\n"
                continue
...
for i in range(0, len(master)):  # Find the Move info for PVP
    if "ENCOUNTER_SETTINGS" in master[i]:
        break
...
while i < len(stringquickpvp[2]):
    if stringquickpvp[2][i] == " ":
        stringquickpvp[2] = stringquickpvp[2][:i] + "1" + stringquickpvp[2][i + 1 :]
    else:
        stringquickpvp[2] = (
            stringquickpvp[2][:i]
            + str(int(stringquickpvp[2][i]) + 1)
            + stringquickpvp[2][i + 1 :]
        )
    i += 2
...
aa.write(stringquick[0])
bb.write(stringquick[1])
cc.write(stringquick[2])
dd.write(stringquick[3])
ee.write(stringquick[4])
ff.write(stringquick[5])
aa.close()
bb.close()
cc.close()
dd.close()
...
aa.write(stringcharge[0])
bb.write(stringcharge[1])
cc.write(stringcharge[2])
dd.write(stringcharge[3])
ee.write(stringcharge[4])
ff.write(stringcharge[5])
aa.close()
bb.close()
cc.close()
dd.close()
...
aa.write(stringquickpvp[0])
bb.write(stringquickpvp[1])
aa.close()
bb.close()

aa = open("move/charge/pvppower.txt", "w", encoding="UTF-8")
bb = open("move/charge/pvpenergy_delta.txt", "w", encoding="UTF-8")
aa.write(stringchargepvp[0])
bb.write(stringchargepvp[1])
aa.close()
bb.close()

s = open("move/quick/pvpduration.txt", "w", encoding="UTF-8")
s.write(stringquickpvp[2])
s.close()
